...
export default function drawEnsembleSplits({ givenSVG, data, margin, ensemble}){
    ...
    /* ------------------------------------------------------------------------ Data */
    const enactedSeat = data.enacted?.[0];
    ...
    if (enactedSeat != null && x(enactedSeat) !== undefined){
        drawEnactedBackground({ svg, x, enactedX: x(enactedSeat), height });
    }
    ...
    if (enactedSeat != null && x(enactedSeat) !== undefined){
        drawEnactedLabel({ svg, x, enactedX: x(enactedSeat), margin, height });
    }
